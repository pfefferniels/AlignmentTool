...
public:
	...
	void ReadFileIpr(string iprFile)
	{ // Note that pitch->sitch is not unique!
		Clear();
		vector<int> v(100);
		vector<double> d(100);
		vector<string> s(100);
	...
	void ReadMIDIFile(string midiFile)
	{
		...
		for (int n = 0; n < midi.content.size(); n += 1)
		{
			...
			if (midi.content[n].mes[0] >= 128 && midi.content[n].mes[0] < 160)
			{ // note-on or note-off event

				curChan = midi.content[n].mes[0] % 16;
				if (midi.content[n].mes[0] >= 144 && midi.content[n].mes[2] > 0)
				{ // note-on
			...
			else if (midi.content[n].mes[0] >= 192 && midi.content[n].mes[0] < 208)
		...
		for (int n = 0; n < midi.content.size(); n += 1)
		{
			if (midi.content[n].mes[0] < 176 || midi.content[n].mes[0] >= 192)
			{
				continue;
			}
