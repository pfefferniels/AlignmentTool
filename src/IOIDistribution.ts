 ...
 * @returns 
 */
const generateIoiDistributionFunction1 = (prob: number, sig: number, mu: number) => {
    return (ioi: number) => {
        return prob * (1 / sig) * Math.exp(-(ioi - mu) / sig) + 0.005 * ioiInsertionDistribution(ioi)
    }
}

const generateIoiDistributionFunction2 = (prob: number, sig: number, mu: number) => {
    return (ioi: number) => {
        return prob * (1 / Math.sqrt(2*Math.PI*sig**2)) * Math.exp(-0.5*Math.pow((ioi-mu) / sig, 2)) + 0.005 * ioiInsertionDistribution(ioi)
    }
}
...
export const ioiDistributionFunctions = new Map<TransitionType, (ioi: number) => number>([
    ...
    [TransitionType.Chord, generateIoiDistributionFunction1(0.995, 0.01, 0)], 
    [TransitionType.Arpeggio, generateIoiDistributionFunction2(0.95, 0.05, 0.05)],
    [TransitionType.ShortAppoggiatura, generateIoiDistributionFunction2(0.95, 0.07, 0.13)],
    [TransitionType.Trill, generateIoiDistributionFunction2(0.95, 0.015, 0.082)]
])
