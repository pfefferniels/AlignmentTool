...
public:
	...
	void ReadFile(string filename)
	{
		...
		while (ifs >> s[0])
		{
			...
			for (int j = 0; j < evt.numClusters; j += 1)
			{
				ifs >> v[10];
				evt.numNotesPerCluster.push_back(v[10]);
			} // endfor j
			evt.sitchesPerCluster.clear();
			evt.sitchesPerCluster.resize(evt.numClusters);
			evt.voicesPerCluster.clear();
			evt.voicesPerCluster.resize(evt.numClusters);
			evt.fmt1IDsPerCluster.clear();
			evt.fmt1IDsPerCluster.resize(evt.numClusters);
			for (int j = 0; j < evt.numClusters; j += 1)
				for (int k = 0; k < evt.numNotesPerCluster[j]; k += 1)
				{
					ifs >> s[10];
					evt.sitchesPerCluster[j].push_back(s[10]);
				} // endfor j,k
			for (int j = 0; j < evt.numClusters; j += 1)
				for (int k = 0; k < evt.numNotesPerCluster[j]; k += 1)
				{
					ifs >> v[10];
					evt.voicesPerCluster[j].push_back(v[10]);
				} // endfor j,k
			for (int j = 0; j < evt.numClusters; j += 1)
				for (int k = 0; k < evt.numNotesPerCluster[j]; k += 1)
				{
					ifs >> s[10];
					evt.fmt1IDsPerCluster[j].push_back(s[10]);
				} // endfor j,k
			getline(ifs, s[99]);
			evts.push_back(evt);
		} // endwhile
	...
	void WriteFile(string filename)
	{
		...
		for (int i = 0; i < evts.size(); i += 1)
		{
			...
			for (int j = 0; j < evts[i].numClusters; j += 1)
				for (int k = 0; k < evts[i].numNotesPerCluster[j]; k += 1)
				{
	...
	void ConvertFromHom(Hom hom)
	{
		...
		{
			if (hom.evts[i].voice > NumOfVoices)
			{
				NumOfVoices = hom.evts[i].voice;
			}
		} // endfor i
		NumOfVoices += 1;
		...
		for (int i = 0; i < NumOfVoices; i += 1)
		{
			...
			{
				if (hom.evts[m].voice != i)
				{
					continue;
				}
				if (hom.evts[m].stime > curstime)
				{
					curstime = hom.evts[m].stime;
					prevStatus = -1;
					status = -1;
				}
				if (hom.evts[m].eventtype == "after-note")
				{
					status = 0;
				}
				else if (hom.evts[m].eventtype == "short-app")
				{
					status = 1;
				}
				else if (hom.evts[m].eventtype == "chord" || hom.evts[m].eventtype == "tremolo")
				{
					status = 2;
				} // endif
				if (status < prevStatus)
				{
					cout << "Error in ordering at " << m << endl;
					assert(false);
				}
				prevStatus = status;
			} // endfor m
...

		{
			...
			for (int i = 0; i < hom.evts.size() + 1; i += 1)
			{
				...
				if (i < hom.evts.size())
				{
					...
					{
						if (hom.evts[i].eventtype == "after-note")
						{
							nevts_an.push_back(hom.evts[i]);
							curType = 0;
						}
						else if (hom.evts[i].eventtype == "short-app")
						{
							nevts_sa.push_back(hom.evts[i]);
							curType = 1;
						}
						else if (hom.evts[i].eventtype == "chord")
						{
							nevts_ch.push_back(hom.evts[i]);
							curType = 2;
						}
						else if (hom.evts[i].eventtype == "tremolo")
						{
							nevts_tm.push_back(hom.evts[i]);
							curType = 3;
						} // endif
				...
				if (nevts_an.size() > 0)
				{
					...
					for (int j = 0; j < nevts_an.size(); j += 1)
					{
						...
						{
							if (nevts_an[0].fmt1IDs[0] != nevts_an[j].fmt1IDs[k])
							{
								multiRef = true;
							}
						} // endfor k
					...
					for (int m = 0; m < NumOfVoices; m += 1)
					{
						if (reorderedNevts[m].size() > Ncluster)
						{
							Ncluster = reorderedNevts[m].size();
						}
						if (reorderedNevts[m].size() > 0)
						{
							NrelevantVoice += 1;
						}
					} // endfor m
					...
					if (NrelevantVoice == 1 && multiRef)
					{
						...
						for (int c = 0; c < Ncluster; c += 1)
						{
							...
							{
								if (reorderedNevts[relevantVoice][c].notetypes[k].find("Arp") == string::npos)
								{
									Nch += 1;
								}
								else
							...
							hmmevt.numNotesPerCluster.clear();
							hmmevt.numNotesPerCluster.resize(hmmevt.numClusters);
							hmmevt.sitchesPerCluster.clear();
							hmmevt.sitchesPerCluster.resize(hmmevt.numClusters);
							hmmevt.voicesPerCluster.clear();
							hmmevt.voicesPerCluster.resize(hmmevt.numClusters);
							hmmevt.fmt1IDsPerCluster.clear();
							hmmevt.fmt1IDsPerCluster.resize(hmmevt.numClusters);
					...
					{ // NrelevantVoice > 1
						...
						for (int c = 0; c < Ncluster; c += 1)
						{
							...
							for (int m = 0; m < NumOfVoices; m += 1)
							{
								...
								{
									if (reorderedNevts[m][c].notetypes[k].find("Arp") == string::npos)
									{
										Nch += 1;
									}
									else
				...
				if (nevts_tm.size() > 0 && chordalAndTrill < 2)
				{
					chordalAndTrill += 2;
				} // endif
				...
				{ // purely chordal
					...
					if (nevts_sa.size() + nevts_ch.size() > 0)
					{
						...
						{
							if (nevts_sa[j].sitches.size() == 0)
							{
								nevts_sa.erase(nevts_sa.begin() + j);
							}
						} // endfor j
						...
						for (int j = 0; j < nevts_sa.size(); j += 1)
						{
							...
							if (firstFmt1ID == "")
							{
								firstFmt1ID = nevts_sa[j].fmt1IDs[0];
							}
							...
							if (nevts_sa.size() > 0)
							{
								...
								{
									if (firstFmt1ID != nevts_sa[j].fmt1IDs[k])
									{
										multiRef = true;
									}
								} // endfor k
						...
						if (NrelevantVoice == 1 && nevts_sa.size() > 0 && multiRef)
						{
							...
							for (int c = 0; c < nevts_sa.size(); c += 1)
							{
								...
								{
									if (nevts_sa[c].notetypes[k].find("Arp") == string::npos)
									{
										Nch += 1;
									}
									else
							...
							{
								if (nevts_ch[0].notetypes[k].find("Arp") == string::npos)
								{
									Nch += 1;
								}
								else
						...
						{ // if NrelevantVoice > 1 or purely chordal
				...
				else if (chordalAndTrill == 2)
				{ // purely trill/tremolo
					...
					for (int j = 0; j < nevts_ch.size(); j += 1)
					{
						for (int k = 0; k < nevts_ch[j].notetypes.size(); k += 1)
						{
							...
							stime = nevts_ch[j].stime;
							if (nevts_ch[j].endstime > endstime)
							{
								endstime = nevts_ch[j].endstime;
							}
					...
					for (int j = 0; j < nevts_tm.size(); j += 1)
					{
						...
						if (nevts_tm[j].endstime > endstime)
						{
							endstime = nevts_tm[j].endstime;
						}
				...
				{ // chord + trill/tremolo
					...
					for (int j = 0; j < nevts_ch.size(); j += 1)
					{
						...
						for (int k = 0; k < nevts_ch[j].notetypes.size(); k += 1)
						{
							if (nevts_ch[j].notetypes[k].find("N") != string::npos)
							{
								withChordal = true;
							}
						} // endfor k
					...
					if (NrelevantVoice == 1 && nevts_sa.size() > 0)
					{
						...
						for (int j = 0; j < nevts_ch.size(); j += 1)
						{
							...
							{
								if (nevts_ch[j].notetypes[k].find("Arp") == string::npos)
								{
									Nch += 1;
								}
								else
					...
					{ // if NrelevantVoice > 1 or purely chordal
						...
						for (int c = 0; c < Ncluster; c += 1)
						{
							...
							for (int m = 0; m < NumOfVoices; m += 1)
							{
								...
								for (int k = 0; k < reorderedNevts[m][c].notetypes.size(); k += 1)
								{
									if (reorderedNevts[m][c].notetypes[k].find("Tr") != string::npos)
									{
										continue;
									}
									ditches.push_back(reorderedNevts[m][c].sitches[k]);
		...
		for (int i = 0; i < evts.size(); i += 1)
		{
			...
			for (int k = 0; k < evts[i].numClusters; k += 1)
				for (int l = 0; l < evts[i].numNotesPerCluster[k]; l += 1)
				{
